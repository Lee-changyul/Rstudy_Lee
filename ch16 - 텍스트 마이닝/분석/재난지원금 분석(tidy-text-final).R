...
texts <- subset(texts, select = -word1)
texts$date <- as.Date(as.character(texts$date))
...
worddf %>%
  filter(TF.IDF >= 5.52 & TF.IDF <= 5.79) %>%
  ggplot() +
  aes(x=reorder(word, TF.IDF), y = TF.IDF) +
  geom_bar(stat='identity') +
  theme_minimal() +
  labs(x="단어", y="가중치") +
  geom_text(aes(label=round(TF.IDF,3)), size=3, hjust=1.25, color='#FFFFFF') +
  coord_flip()

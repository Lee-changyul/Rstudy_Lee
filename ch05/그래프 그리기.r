...
# 그래프로(테이블) 된 것만 barplot 으로 그릴 수 있다. table로 정리한 후에 그려야 한다. - 원자료 사용못한다. 
# 기본 막대그래프 
...
pie(grade,
    main = "학점별 분포",
    init.angle = 180, # 처음 각도를 0도에서 시작하라. 
    col = rainbow(length(grade))) # grade 테이블의 행의 갯수 

legend(0 ,0, # 범례지정함수는 그래프와 별도로 입력할 수 있음 1,1은 위치지정 ()
       rownames(grade),
       cex = 0.5, # 범례 글자 크기 
...
pre.p <- prop.table(pre.t) # 만들어 놓은 테이블 데이터를 활용한다.
...
wgt <- transform(wgt,
                 wgt.cut = cut(weight,
                               ...
                               labels=c("~45미만", "45이상~50미만","50이상~55미만","55이상~60미만",
                                        "60이상~65미만","65이상~70미만","70이상~")))

# 범주형 자료로 변환 (ifelse이용)
wgt<- transform(wgt,
                wgt.if = ifelse(weight < 45, "~ 45미만",
                                ifelse(weight >= 45 & weight < 50, "45이상~50미만",
                                       ifelse(weight >= 50 & weight < 55, "50이상~55미만",
                                              ifelse(weight >= 55 & weight < 60, "55이상~60미만",
                                                     ifelse(weight >= 60 & weight < 65, "60이상~65미만",
                                                            ifelse(weight >= 65 & weight < 70, "65이상~70미만","70이상~"
                                                            )
                                                     )
                                              )
                                       )
...
barplot(game.t,
        col=c("darkblue"),
        main = "연속변수 평균",
        names.arg = c(colnames(game.t)), # 이거 의미 -어레인. 이름을 넣는데, 컬럼 순서대로 넣으라는 뜻
        ylim = c(0,4))
